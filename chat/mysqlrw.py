...
class MysqlRw:
    ...
    def read_task(self, nickname):
        try:
            cursor = self.connect_mysql()
            query = "SELECT id, sleep_time, event FROM TimeTask WHERE username = %s order by sleep_time"
            ...
            if result:
                ...
                return self.title + result_string
            else:
    ...
    def write_task(self, nick_name, task_time, task):
        try:
            ...
            self.reusest = "定时任务创建成功\n" + self.title + f"{inserted_row[0]}   {inserted_row[1]}   {inserted_row[2]}"
            return self.reusest
    ...
    def update_task(self, username, id, new_time=None, new_task=None):
        try:
            ...
            self.reusest = "任务更新成功\n" + self.title + f"{updated_row[0]}   {updated_row[1]}   {updated_row[2]}"
            return self.reusest
    ...
    def del_task(self, username, id):
        ...
        try:
            ...
            if isinstance(id, list):
                ...
                self.reusest = f"删除成功\n" + self.title + f"{deleted_row[0]}   {deleted_row[1]}   {deleted_row[2]}"
                return self.reusest
    ...
    def return_time(self, course_id):
        ...
        for row in result:
            ...
            # 将结束时间提前一分钟
            end_row =  datetime.datetime.today() - datetime.timedelta(minutes=-1)
            ...
            elif end_row.strftime("%H:%M") == end_minutes:
    ...
    def get_group_users(self, user_id):
        try:
            ...
            if result:
                result_str = ' '.join(str(item) for item in result[0])
                logger.info('get_group--->%s', result_str)
                return result_str
            else:
                return '没有指定成员'
